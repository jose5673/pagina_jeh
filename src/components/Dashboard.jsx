...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    //configIntervalRef.current = setInterval(cargarConfiguracion, 3000000);
  ...
  const obtenerValoresGauges = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Simplemente actualizar el estado directamente
  ...
  // Función de utilidad para evitar logs repetidos
  const logDebounced = (() => {
    let lastLog = {};
    return (message, data, minInterval = 1000) => {
      const key = message;
      const now = Date.now();
      if (!lastLog[key] || (now - lastLog[key]) > minInterval) {
        ...
        {loading && Object.keys(displayValues).length === 0 ? (
          ...
          <div className={`gauge-dashboard-container ${!isConnected ? 'opacity-75' : ''}`}>
            {Array.isArray(gaugeConfig) && gaugeConfig.map(gauge => {
              ...
              return (
                <Gauge
                  key={`${gauge.id}-${lastConfigUpdateRef.current}`}
                  id={gauge.id}
                  title={gauge.name}
                  unit={gauge.unit}
